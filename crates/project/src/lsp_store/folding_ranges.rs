...
use crate::lsp_store::{FoldingRanges, LspStore};
...
impl LspStore {
    ...
    pub fn fetch_folding_ranges(
        ...
        if let Some((updating_for, running_update)) = &folding_lsp_data.ranges_update {
            if !version_queried_for.changed_since(updating_for) {
                ...
                return cx.background_spawn(async move {
                    let result: Result<FoldingRanges, Arc<anyhow::Error>> = running.await;
                    result.map(|r| r.ranges).unwrap_or_default()
                });
            }
        }
                    ...
                    .update(cx, |lsp_store, cx| {
                        ...
                        FoldingRanges {
                            ranges: folding
                                ...
                                .sorted_by(|a, b| a.start.cmp(&b.start, &snapshot))
                                .collect(),
                            cache_version: Some(folding.cache_version),
                        }
                    })
                    .map_err(Arc::new)
            })
        ...
        cx.background_spawn(async move {
            let result: Result<FoldingRanges, Arc<anyhow::Error>> = new_task.await;
            result.map(|r| r.ranges).unwrap_or_default()
        })
    }
