...
use serde_json::json;
use settings::{
    InlayHintSettingsContent, InlineBlameSettings, LspFoldingRanges, SemanticTokens, SettingsStore,
};
use std::{
...
#[gpui::test]
async fn test_lsp_folding_ranges(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    let mut server = TestServer::start(cx_a.executor()).await;
    let executor = cx_a.executor();
    let client_a = server.create_client(cx_a, "user_a").await;
    let client_b = server.create_client(cx_b, "user_b").await;
    ...
    cx_b.update(|_, cx| {
        SettingsStore::update_global(cx, |store, cx| {
            store.update_user_settings(cx, |settings| {
                settings.project.all_languages.defaults.lsp_folding_ranges =
                    Some(LspFoldingRanges::On);
            });
        });
    });
    executor.advance_clock(LSP_REQUEST_DEBOUNCE_TIMEOUT);
